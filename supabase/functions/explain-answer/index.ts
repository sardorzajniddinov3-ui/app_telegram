 ...
 */
async function getAIExplanation(prompt: string, apiKey: string, imageUrl?: string | null): Promise<string> {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      ...
      if (imageUrl) {
        try {
          console.log('🖼️ Загружаю изображение для анализа:', imageUrl);
          const imageResponse = await fetch(imageUrl);
          if (imageResponse.ok) {
            const imageBuffer = await imageResponse.arrayBuffer();
            const base64Image = btoa(String.fromCharCode(...new Uint8Array(imageBuffer)));
            
            // Определяем MIME тип по URL или заголовкам
            ...
            console.log('✅ Изображение загружено и добавлено в запрос');
          } else {
            console.warn('⚠️ Не удалось загрузить изображение:', imageResponse.status);
          }
        } catch (imageError) {
...
Deno.serve(async (req) => {
  ...
  try {
    const { question, wrongAnswer, correctAnswer, imageUrl } = await req.json()
    ...
    const prompt = imageUrl
      ? `Ты — умный репетитор. Пользователь ошибся в тесте по правилам дорожного движения. 

ВАЖНО: К вопросу прикреплено изображение. Внимательно проанализируй его перед ответом.
...
Твоя задача:
1. Проанализируй изображение и опиши, что на нём изображено (дорожный знак, разметка, ситуация на дороге и т.д.)
2. Объясни, почему выбранный пользователем ответ неверен, ссылаясь на детали изображения
3. Объясни, почему правильный ответ корректен, также ссылаясь на изображение
4. Дай краткую подсказку, как не ошибиться в подобных вопросах

Используй русский язык. Будь кратким и понятным.`
      : `Ты — умный репетитор. Пользователь ошибся в тесте. Кратко и доступно объясни, почему его вариант неверен и какой ответ правильный. Используй русский язык.
...
Правильный ответ: ${correctAnswer}`;
    ...
    console.log('🔄 Запрос к ИИ с автоматическим переключением моделей (Fallback)', imageUrl ? '(с изображением)' : '')
    const explanation = await getAIExplanation(prompt, apiKey, imageUrl)
